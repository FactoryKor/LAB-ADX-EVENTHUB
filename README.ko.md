...
> 세션 사이에는 `Scripts_Linux/10_adx_stop_start.sh stop`(또는 `Scripts_windows\10_adx_stop_start.ps1 -Action stop`), 종료 시에는 cleanup 스크립트로 정리하세요.
...
├─ Scripts_Linux/            # Bash 프로비저닝 (Cloud Shell / WSL / Linux / macOS)
│  ├─ 00_create_lab.sh       # 전체 랩 생성; lab.env 생성
│  ├─ 10_adx_stop_start.sh   # ADX 클러스터 start/stop (비용 절감)
│  └─ 99_cleanup.sh          # 리소스 그룹 삭제
├─ Scripts_windows/          # PowerShell 프로비저닝 (Windows)
│  ├─ 00_create_lab.ps1      # 전체 랩 생성; lab.ps1 생성
│  ├─ 10_adx_stop_start.ps1
│  └─ 99_cleanup.ps1
├─ 01_send_events.py         # Producer: normal/skew/burst/badjson/mismatch, --backfill-hours, --auth aad
├─ 02_slow_consumer.py       # consumer lag 재현용 느린 Consumer (Blob 체크포인트, --auth aad)
├─ 03_verify.py              # 검증 하네스: KQL 점검 -> PASS/WARN/INFO
├─ 03_adx_queries.kql        # 수동 진단 쿼리 (스큐·실패·지연·캐시/콜드)
├─ 04_adx_bulk_generate.kql  # 쿼리/스캔/캐시 성능용 대량 합성 데이터
├─ requirements.txt          # Python 의존성
├─ README.md / README.ko.md  # 이 가이드(영어 / 한국어)
└─ (생성됨) lab.env / lab.ps1   # 생성 스크립트가 쓰는 환경 변수 — gitignore 대상
```

**생성 스크립트**(`Scripts_Linux/00_create_lab.sh` 또는 `Scripts_windows/00_create_lab.ps1`)가
Event Hubs, ADX 클러스터/DB, Log Analytics, Storage, 진단 설정, RBAC, 테이블 + JSON 매핑,
ingestion batching 정책을 모두 프로비저닝합니다.

> **모든 명령은 저장소 루트(repo root)에서 실행**하세요. 그래야 `lab.env` / `lab.ps1`과 생성되는
> `adx_init.kql`이 Python 스크립트 옆에 놓입니다.
...
- **Azure CLI**. `Scripts_Linux/`의 **Bash** 스크립트(Azure Cloud Shell / WSL / Linux / macOS) **또는** `Scripts_windows/`의 **PowerShell** 스크립트(Windows)를 사용하세요.
- **Python 3.10+**.
...
./Scripts_Linux/00_create_lab.sh   # 성공 시 ./lab.env 생성
...
`.sh` 스크립트에는 PowerShell 버전(`.ps1`)이 함께 제공됩니다. Azure CLI 설치 + `az login` 후 PowerShell에서 실행하세요:

```powershell
# 1) 랩 생성 (.\lab.ps1 생성됨). 저장소 루트에서 실행.
.\Scripts_windows\00_create_lab.ps1

# 2) 환경 변수 로드 + Python 환경
. .\lab.ps1                       # dot-source (맨 앞의 점 + 공백 주의)
python -m venv .venv
...
```
> 스크립트 실행이 차단되면: `Set-ExecutionPolicy -Scope Process -ExecutionPolicy Bypass`.
...
생성 스크립트 실행 전에 설정(모두 선택 사항, 합리적 기본값 제공):
...
- **Windows에서 `.sh`가 실행 안 됨** → `Scripts_windows/`의 PowerShell 스크립트를 사용하거나, Bash 스크립트를 Azure Cloud Shell / WSL에서 실행.
